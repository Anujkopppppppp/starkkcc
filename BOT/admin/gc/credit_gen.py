...
Contact the bot owner @Stxthunder!
</b>"""
...
async def generate_redeem_codes(client, message):
    try:
        ...
        for _ in range(amount):
            redeem_code = f"THUNDER-{gcgenfunc()}{gcgenfunc()}{gcgenfunc()}-OP"
            await insert_pm(redeem_code)
            response_text += f"➔ <code>{redeem_code}</code>\n"

        response_text += """<b>\nYou can redeem this code using this command: /redeem THUNDER-XXXX-OP</b>"""
